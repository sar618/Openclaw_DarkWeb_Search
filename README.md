...
Prerequisites
Tor must be installed and running as a SOCKS5 proxy on 127.0.0.1:9050
curl must be available for making requests through Tor
...
Direct search queries - Search for the topic combined with terms like:

"<topic>" site:.onion
"<topic>" .onion link
"<topic>" hidden service tor
"<topic>" onion address
Known aggregators - Query Tor-accessible search engines via SOCKS proxy:
...
Extract .onion URLs from all results using pattern:
...
Extract the page title, description, and key content
Extract any additional .onion links found on the page (for deeper crawling if requested)
Record the HTTP status code and basic metadata
...
Safety Guidelines
Never submit forms or interact with services - only read/fetch page content
Never download files from .onion sites - only retrieve HTML content
Never create accounts or authenticate on any dark web service
Always use --socks5-hostname (not --socks5) to prevent DNS leaks
Rate limit requests - wait 2-3 seconds between requests to avoid overloading Tor circuits
Timeout aggressively - .onion sites are often slow or offline; don't wait more than 30 seconds
Log all accessed URLs for audit trail purposes
Respect robots.txt where present on .onion sites
Never attempt to deanonymize hidden service operators or users
IP Protection Rules
...
Verify Tor is running (start if needed)
Search surface web: "AcmeCorp" .onion, "AcmeCorp" data breach tor hidden service
Search Ahmia: https://ahmia.fi/search/?q=AcmeCorp+data+breach
Collect all discovered .onion URLs
Fetch each .onion site through Tor proxy
Extract and summarize relevant content
Present structured report with findings
Troubleshooting
